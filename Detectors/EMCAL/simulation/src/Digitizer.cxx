...
void Digitizer::init()
{
  mSimParam = 0;//&(o2::emcal::SimParam::Instance());
...
void Digitizer::hitToDigits(const Hit& hit)
{
  ...
  Double_t energy = hit.GetEnergyLoss();
    ...
    //LOG(WARNING)<<"QQQ HITENERGY "<<mEventTime<<" "<<energy<<" "<<mPhase<<" "<<mSignalFractionInTimeBins.at(mPhase).size();
  ...
  if (mSimulateTimeResponse) {
    for (int j = 0; j < mSignalFractionInTimeBins.at(mPhase).size(); j++) {
      ...
      mTempDigitVector.push_back(digit);
        //LOG(WARNING)<<"QQQ DIGITENERGY "<<digit.getTimeStamp()<<" "<<val;
    }
  } else {
