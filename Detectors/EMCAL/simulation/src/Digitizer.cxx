...
#include "EMCALBase/LabeledDigit.h"
  ...
  for (auto hit : hits) {
    try {
      ...
      for (auto digit : mTempDigitVector) {
        ...
        o2::emcal::MCLabel label(hit.GetTrackID(), mCurrEvID, mCurrSrcID, false, 1.0);
...
void Digitizer::addNoiseDigits()
{
  for (int id = 0; id <= mGeometry->GetNCells(); id++) {
    ...
    o2::emcal::MCLabel label(true, 1.0);
    LabeledDigit d(digit, label);
...
void Digitizer::fillOutputContainer(std::vector<Digit>& digits)
{
  ...
  for (auto d : l) {
    ...
    std::vector<o2::emcal::Label> labels = d.getLabels();
