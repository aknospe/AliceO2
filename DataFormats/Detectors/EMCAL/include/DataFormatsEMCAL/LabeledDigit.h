...
class LabeledDigit
{
 public:
  ...
  LabeledDigit(Digit digit, o2::MCCompLabel label);
  LabeledDigit(Short_t tower, Double_t energy, Double_t time, o2::MCCompLabel label);
  ~LabeledDigit() = default; // override
    
  void setDigit(Digit d) {mDigit = d; }
  Digit getDigit() const { return mDigit; }
    
  void addLabel(o2::MCCompLabel l) { mLabels.push_back(l); }
  Int_t getNumberOfLabels() const { return mLabels.size(); }
  std::vector<MCCompLabel> getLabels() const { return mLabels; }
  ...
  LabeledDigit& operator+=(const LabeledDigit& other);              // Adds energy of other digit to this digit.
  friend LabeledDigit operator+(LabeledDigit lhs, const LabeledDigit& rhs) // Adds energy of two digits.
  {
    lhs += rhs;
    return lhs;
  }
 ...
 private:
  ...
  Digit mDigit;                     ///< Digit
  std::vector<o2::MCCompLabel> mLabels; ///< Labels
